...
<?php
        if (isset($_GET['page'])) {
        $page_search = $_GET['page'];
        $searchq = $_GET['query'];
        $posts_url=$_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'];
        $searchq=preg_replace("#[^0-9a-z]#i", "", $searchq);
  
        $no_of_records_search_per_page = 4;
        $offset_search = ($page_search-1) * $no_of_records_search_per_page;
        
        $total_pages_search_sql = "SELECT COUNT(*) FROM post WHERE keyword LIKE '%$searchq%' OR post_title LIKE '%$searchq%' OR description LIKE '%$searchq%' ";
        ...
        $no_of_records_search_per_page = 4;
        $total_search_pages = ceil($total_search_rows / $no_of_records_search_per_page);
        $page_search = $total_search_pages;
        $offset_search = ($page_search-1) * $no_of_records_search_per_page;
        }
  $search_sql="SELECT * FROM post WHERE keyword LIKE '%$searchq%' OR post_title LIKE '%$searchq%' OR description LIKE '%$searchq%' LIMIT $offset_search, $no_of_records_search_per_page ";
...
>
            <?php 
            $minus_page=$page_search - 1;
            $plus_page=$page_search + 1;

            $older='search?query='.$searchq.'&page='.$minus_page;
            $newer='search?query='.$searchq.'&page='.$plus_page;
